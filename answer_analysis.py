...
from 价格 import numberdetection


from 菜品种类 import analysis_type
from 地点 import analysis_didian
...
def analysis(answer, groove,question_index):
    print('groove：',groove)
    print('question_type:'+str(groove[3]))
    """
    ...
    if question_index !=1 or question_type !=0 :
        slot_occasion = analysis_type(groove, answer)
    if question_index != 1 or question_type != 2:
        slot_area = numberdetection(answer, groove)
    if question_index != 1 or question_type != 1:
        slot_pm = analysis_didian(groove, answer)
    print('菜品:', slot_occasion)
    print('价格:', slot_area)
    print('地点:', slot_pm)
...
def is_about(answer):
    ...
    key_words = ['空气', '净化', '霾', '这款', '产品', '性能', '效果', '提升', "作用", '对比', '介绍', '价格', '价位', '优惠', "活动", "机器", "机子",
                 ...
                 '能用']
    key_words = ['味道','口味','价格','地点','附近','美味','实惠','菜','店','香','种类','口感']
