...
#    patternnumber = re.compile('([一二三四五六七八九零十百千万亿]+|[0-9]+[]*[0-9])')
    ...
    largerpattern1  = re.compile('超过|超了|向上')
    largerpattern2  = re.compile('超|多|大')
    disturbpattern1 = re.compile('元|块|台|箱')  
    disturbpattern2 = re.compile('价格|价钱')
    unitpattern1 = re.compile('平')
    unitpattern2 = re.compile('面积|占地|空间|大小')
    
    
    allr = patternnumber.findall(userstr)
    allr = [lr for lr in allr if lr != '']
    numbers = []
    lesslarger = []
    isdistrub = []
    isarea = []
...
# print('啦啦啦')
    ...
    if down > 100 or  rightarea > 110 or up <1:
        return 3    
    if rightarea >0 and rightarea <=50:
        return 1    
    if rightarea > 50 and rightarea <= 110:
        return 2
    
    if down < 50 and up > 0:
        return 1
    
    if down > 50 and down < 110:
        return 2   
