...
from model_ import SequenceLabelingModel_
...
# 加载配置文件
with open(r'G:\project QA\前期版本\识别正向、负向食物\config.yml', 'rb') as file_config:
...
config['data_params']['voc_params']['f1']['path'] = os.path.join(NER_ROOT,
                                                                 config['data_params']['voc_params']['f1']['path'])
config['data_params']['voc_params']['f2']['path'] = os.path.join(NER_ROOT,
                                                                 config['data_params']['voc_params']['f2']['path'])
config['data_params']['voc_params']['label']['path'] = os.path.join(NER_ROOT,
                                                                    config['data_params']['voc_params']['label'][
                                                                        'path'])
config['model_params']['embed_params']['f1']['path'] = os.path.join(NER_ROOT,
                                                                    config['model_params']['embed_params']['f1'][
                                                                        'path'])
config['model_params']['embed_params']['f1']['path_pre_train'] = os.path.join(NER_ROOT,
                                                                              config['model_params']['embed_params'][
                                                                                  'f1']['path_pre_train'])
...
for feature_name in feature_names:
    ...
    if path_pre_train:
        # path_pre_train = 'G:\\project QA\\前期版本\\识别正向、负向食物\\'+path_pre_train
...
#with tf.variable_scope('studybrother'):
model = SequenceLabelingModel_(
...
def predict_(string):
    ...
    while (5 in seq[0][beginnumber:]):
        ...
        if 5 in seq[0][beginnumber:] and 6 in seq[0][beginnumber:]:
            ...
            Food = lab[place_5:place_6]
            Food = ''.join(Food)
            food = food + Food + ','
            beginnumber = beginnumber + place_6
        else:
            place_5 = beginnumber + seq[0][beginnumber:].index(5)
            Food = lab[place_5:(place_5 + 1)]
            Food = ''.join(Food)
            food = food + Food + ','
    ...
    while (3 in seq[0][beginnumber:]):
        Food_n = ''
        if 3 in seq[0][beginnumber:] and 4 in seq[0][beginnumber:]:
            place_3 = beginnumber + seq[0][beginnumber:].index(3)
            place_4 = beginnumber + seq[0][beginnumber:].index(4) + 1
            Food_n = lab[place_3:place_4]
            Food_n = ''.join(Food_n)
            food_n = food_n + Food_n + ','
            beginnumber = beginnumber + place_4
        else:
            place_3 = beginnumber + seq[0][beginnumber:].index(3)
            Food_n = lab[place_3:(place_3 + 1)]
            Food_n = ''.join(Food_n)
            food_n = food_n + Food_n + ','
            beginnumber = beginnumber + place_3 + 1
    beginnumber = 0
    taste_n = ''
    while 8 in seq[0][beginnumber:]:
        Taste_n = ''
        if 8 in seq[0][beginnumber:] and 11 in seq[0][beginnumber:]:
            place_8 = beginnumber + seq[0][beginnumber:].index(8)
            place_11 = beginnumber + seq[0][beginnumber:].index(11) + 1
            Taste_n = lab[place_8:place_11]
            Taste_n = ''.join(Taste_n)
            taste_n = taste_n + Taste_n + ','
            beginnumber = beginnumber + place_11
        else:
            place_8 = beginnumber + seq[0][beginnumber:].index(8)
            Taste_n = lab[place_8:(place_8 + 1)]
            Taste_n = ''.join(Taste_n)
            taste_n = taste_n + Taste_n + ','
            beginnumber = beginnumber + place_8 + 1
...
def food_taste_extract(string):
    ...
    for i in range(len(string_l)):
        if string_l[i] == '':
            ...
            food_l, taste_l, food_n_l, taste_n_l = predict_(string_l[i])
            food = (food + food_l + ',')[:-1]
