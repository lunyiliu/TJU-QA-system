...
# -*- encoding: utf-8 -*-
"""
模型输入：f1（每个字的id）、f2（每个字的词性的id）向量
模型输出：label(每个字的label的id)
静态资源准备：
(1)vocs字典: [{字:id},{词性:id},{label：id}]
(2)embedding_matrix:
自己的数据->vocs字典------|(preprocess.py)
		|						   |
		|->embed （字符-词向量）---|->embedding_matrix（id-词向量）
顺序：
	data_prepare.py->preprocessing.py->train.py
    fine_tune NER模型
	 得到新的vocs preprocess.py/build_vocabulary()
	 得到新的embeddings_matrix preprocess.py
	 修改配置文件
	 训练
"""
import yaml
