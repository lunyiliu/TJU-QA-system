...
#        line = file_r.readline()
    ...
    modelWord2Vec=Word2Vec.load('Model/Word2vec_model.pkl')
    gensim_dict = Dictionary()
    gensim_dict.doc2bow(modelWord2Vec.wv.vocab.keys(),allow_update=True)
    w2indx = {v: k+2 for k, v in gensim_dict.items()}#所有频数超过10的词语的索引
    embedding = {word: modelWord2Vec[word] for word in w2indx.keys()}#所有频数超过10的词
    vec_dim = 64
